...
use nitro_cli::common::commands_parser::{
    BuildEnclavesArgs, ConsoleArgs, DescribeArgs, DescribeEnclavesArgs, EmptyArgs, ExplainArgs,
    PcrArgs, RunEnclavesArgs, SignArgs, TerminateEnclavesArgs,
};
...
use nitro_cli::{
    ...
    get_file_pcr, new_enclave_name, new_nitro_cli_failure, sign_eif_file, terminate_all_enclaves,
};
    ...
    match args.subcommand() {
        ...
        ("describe-eif", Some(args)) => {
            let describe_args = DescribeArgs::new_with(args)
                .map_err(|e| {
                    e.add_subaction("Failed to construct describe arguments".to_string())
                        .set_action(FILE_PCR_STR.to_string())
                })
                .ok_or_exit_with_errno(None);
            describe_eif(describe_args)
                .map_err(|e| {
        ...
        ("sign-eif", Some(args)) => {
            let sign_args = SignArgs::new_with(args)
                .map_err(|e| {
                    e.add_subaction("Failed to construct Sign arguments".to_string())
                        .set_action(SIGN_EIF_STR.to_string())
                })
                .ok_or_exit_with_errno(None);

            sign_eif_file(sign_args)
                .map_err(|e| {
                    e.add_subaction("Failed to sign EIF file".to_string())
                        .set_action(SIGN_EIF_STR.to_string())
                })
                .ok_or_exit_with_errno(None);
        }
